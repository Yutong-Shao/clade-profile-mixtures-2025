...
LG_MEOW60highfixPMSF <- read.table("D:/8701/clade-profile-mixtures-2025/data/tardigrades (Giacomelli et al., 2024)/output_40taxa/LG-MEOW60high-PMSF/diversity_scores_bootstrapped_data.txt_gaps", header = TRUE, sep = "\t", stringsAsFactors = FALSE)

LG_MEOW60highoptPMSF <- read.table("D:/8701/clade-profile-mixtures-2025/data/tardigrades (Giacomelli et al., 2024)/output_40taxa/LG-MEOW60highopt-PMSF/diversity_scores_bootstrapped_data.txt_gaps", header = TRUE, sep = "\t", stringsAsFactors = FALSE)
...
GTR20_MEOW60highoptPMSF <- read.table("D:/8701/clade-profile-mixtures-2025/data/tardigrades (Giacomelli et al., 2024)/output_40taxa/GTR20-MEOW60opt-PMSF/diversity_scores_bootstrapped_data.txt_gaps", header = TRUE, sep = "\t", stringsAsFactors = FALSE)
...
LG_MEOW60highfixPMSF$dataset <- "LG+MEOW60fix-PMSF"
LG_MEOW60highoptPMSF$dataset <- "LG+MEOW60opt-PMSF"
...
GTR20_MEOW60highoptPMSF$dataset <- "GTR20+MEOW60opt-PMSF"

combined_data <- rbind(Poisson_CAT_PMSF, Poisson_C60fix, Poisson_C60opt, Poisson_C10fixPMSF, Poisson_C10optPMSF, Poisson_C60fixPMSF, Poisson_C60optPMSF, Poisson_UDM8fixPMSF, Poisson_UDM8optPMSF, Poisson_UDM64fixPMSF, Poisson_UDM64optPMSF, Poisson_UDM128fixPMSF, LG_C60fix, LG_C10fixPMSF, LG_C10optPMSF, LG_C60fixPMSF, LG_C60optPMSF, LG_UDM8fixPMSF, LG_UDM8optPMSF, LG_UDM64fixPMSF, LG_UDM64optPMSF, LG_UDM128fixPMSF, LG_MEOW60highfixPMSF, LG_MEOW60highoptPMSF, GTR20_C10fixPMSF, GTR20_C10optPMSF, GTR20_C60fixPMSF, GTR20_C60optPMSF, GTR20_UDM8fixPMSF, GTR20_UDM8optPMSF, GTR20_UDM64fixPMSF, GTR20_UDM64optPMSF, GTR20_UDM128fixPMSF, GTR20_UDM128optPMSF, GTR20_MEOW60highoptPMSF)
```
...
combined_data <- combined_data %>%
  mutate(group = case_when(
    ...
    dataset %in% c("Poisson+UDM8fix-PMSF", "Poisson+UDM8opt-PMSF", "Poisson+UDM64fix-PMSF", "Poisson+UDM64opt-PMSF", "Poisson+UDM128fix-PMSF", "LG+UDM8fix-PMSF", "LG+UDM8opt-PMSF", "LG+UDM64fix-PMSF", "LG+UDM64opt-PMSF", "LG+UDM128fix-PMSF", "GTR20+UDM8fix-PMSF", "GTR20+UDM8opt-PMSF", "GTR20+UDM64fix-PMSF", "GTR20+UDM64opt-PMSF", "GTR20+UDM128fix-PMSF", "GTR20+UDM128opt-PMSF") ~ "UDM-PMSF",
    
    dataset %in% c("LG+MEOW60opt-PMSF", "LG+MEOW60fix-PMSF", "GTR20+MEOW60opt-PMSF") ~ "MEOW-PMSF",
...
group_lines <- data.frame(
  ...
  x_start = c(1.6, 12.6, 28.6, 32.6),
  x_end   = c(12.4, 28.4, 32.4, 35.4),
...
ggplot(combined_data, aes(x = dataset, y = as.numeric(diversity), fill = dataset)) +
  ...
  geom_vline(xintercept = c(1.5, 12.5, 28.5, 32.5), linetype = "dotted", color = "grey70", linewidth = 0.5) +
...
combined_data <- combined_data %>%
  mutate(
    # Step 1: 添加 model 大组信息
    model = case_when(
      dataset == "Poisson-CAT-PMSF" ~ "CAT-PMSF",
      dataset %in% c(
        "GTR20+C10fix-PMSF", "GTR20+C10opt-PMSF", "GTR20+C60fix-PMSF", "GTR20+C60opt-PMSF",
        "LG+C10fix-PMSF", "LG+C10opt-PMSF", "LG+C60fix-PMSF", "LG+C60opt-PMSF",
        "Poisson+C10fix-PMSF", "Poisson+C10opt-PMSF", "Poisson+C60fix-PMSF", "Poisson+C60opt-PMSF"
      ) ~ "CXX-PMSF",
      dataset %in% c(
        "Poisson+UDM8fix-PMSF", "Poisson+UDM8opt-PMSF", "Poisson+UDM64fix-PMSF", "Poisson+UDM64opt-PMSF", "Poisson+UDM128fix-PMSF",
        "LG+UDM8fix-PMSF", "LG+UDM8opt-PMSF", "LG+UDM64fix-PMSF", "LG+UDM64opt-PMSF", "LG+UDM128fix-PMSF",
        "GTR20+UDM8fix-PMSF", "GTR20+UDM8opt-PMSF", "GTR20+UDM64fix-PMSF", "GTR20+UDM64opt-PMSF", "GTR20+UDM128fix-PMSF", "GTR20+UDM128opt-PMSF"
      ) ~ "UDM-PMSF",
      dataset %in% c("LG+MEOW60opt-PMSF", "LG+MEOW60fix-PMSF", "GTR20+MEOW60opt-PMSF") ~ "MEOW-PMSF",
      dataset %in% c("Poisson+C60fix", "Poisson+C60opt", "LG+C60fix") ~ "C60",
      TRUE ~ NA_character_
...
filtered_data <- combined_data %>%
  mutate(
    profile = case_when(
      model == "CXX-PMSF" & profile %in% c("C10", "C60") ~ profile,
      model == "UDM-PMSF" & profile %in% c("UDM8", "UDM64", "UDM128") ~ profile,
      model == "MEOW-PMSF" & profile == "MEOW60" ~ profile,
      model == "C60" & profile == "C60" ~ profile,
      model == "CAT-PMSF" & profile == "default" ~ profile,
      TRUE ~ NA_character_
    )
  ) %>%
  filter(!is.na(profile)) %>%
  mutate(
    model = factor(model, levels = c("CAT-PMSF", "CXX-PMSF", "UDM-PMSF", "MEOW-PMSF", "C60")),
